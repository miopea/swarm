    ...
    def unarchive(self, task_id: str) -> bool:
        ...
        with self._lock:
            ...
            if task is None:
                return False
            if task.status is TaskStatus.MIGRATED:
                # Same ownership guard as remove/archive/remove_tasks (#12). Without it
                # restore would be the ONE mutator that can put a task Swarm Next owns
                # back on this board — a stale duplicate of live work elsewhere.
                _log.warning("cannot restore migrated task %s — Swarm Next owns it", task_id)
                return False
